    ...
    def load_image(self, i):
        ...
        if im is None:  # not cached in RAM
            ...
            if r != 1:  # if sizes are not equal
                ...
                dsize = (math.ceil(w0 * r), math.ceil(h0 * r))
                if im.shape[2] == 6:
                    im1 = cv2.resize(im[:, :, :3], dsize, interpolation=interp)
                    im2 = cv2.resize(im[:, :, 3:], dsize, interpolation=interp)
                    im = np.concatenate((im1, im2), axis=2)
                else:
                    im = cv2.resize(im, dsize, interpolation=interp)
